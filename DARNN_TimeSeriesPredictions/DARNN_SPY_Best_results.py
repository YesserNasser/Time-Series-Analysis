# -*- coding: utf-8 -*-
"""
Created on Fri Jul 26 11:33:51 2019
...
def data_processing(data,target_list) -> Tuple[TrainData,StandardScaler]:
    ...
    return TrainData(features, targs), scale
# ============================= Build DA-RNN ==================================
...
def prep_train_data(batch_idx: np.ndarray, 
    ...
    features = np.zeros((len(batch_idx), t_cfg.T -1, train_data.feats.shape[1]))
    y_history = np.zeros((len(batch_idx), t_cfg.T -1, train_data.targs.shape[1]))
    y_target = train_data.targs[batch_idx + t_cfg.T]
    
    for b_i, b_idx in enumerate(batch_idx):
        b_slc = slice(b_idx, b_idx+t_cfg.T - 1)
        features[b_i, :, :] = train_data.feats[b_slc,:]
        y_history[b_i, :] = train_data.targs[b_slc]
        
    return features, y_history, y_target
# =========================== Train_iteration =================================
def train_iteration(t_net: DaRnnNet, 
                    loss_func: typing.Callable, 
                    X, 
                    y_history, 
                    y_target):
    ...
    return loss.item()
# =========================== adjust learning =================================
def adjust_learning_rate(t_net:DaRnnNet, n_iter: int):
...
def train(net: DaRnnNet, train_data: TrainData, t_cfg: TrainConfig, n_epochs=10):
    ...
    for e_i in range(n_epochs):
        ...
        if e_i % 1 == 0:      
            ...
            plt.plot(range(t_cfg.T + len(y_train_pred), len(train_data.targs) + 1), y_test_pred, label= f'Predicted - Test_epoch_{e_i}')
            plt.legend(loc='upper left')
            plt.grid()
    return iter_losses, epoch_losses
# =============================================================================
# =============================================================================    
#raw_data = pd.read_csv('df_data_Least_SPY.csv')
#target_list = ('SPY',)   
#    
#data, scale =  data_processing(raw_data,target_list)   
#
#da_rnn_args = {'batch_size':128,'T':10} 
#   
#config, model = da_rnn(data, n_targs=len(target_list), learning_rate=.01, **da_rnn_args)      
#iter_loss, epoch_loss = train(model, data, config, n_epochs = 101)
...
da_rnn_args = {'batch_size':32,'T':6} 
   
config, model = da_rnn(data, n_targs=len(target_list), learning_rate=.002, **da_rnn_args)      
iter_loss, epoch_loss = train(model, data, config, n_epochs = 71)
final_y_pred = predict(model, data, config.train_size, config.batch_size, config.T)
## ============================================================================
## ============================================================================
#raw_data = pd.read_csv('df_data_Most_SPY.csv')
#target_list = ('SPY',)   
#    
#data, scale =  data_processing(raw_data,target_list)   
#
...
#   
#config, model = da_rnn(data, n_targs=len(target_list), learning_rate=.01, **da_rnn_args)      
#iter_loss, epoch_loss = train(model, data, config, n_epochs = 40)
#final_y_pred = predict(model, data, config.train_size, config.batch_size, config.T)
#
plt.figure()
